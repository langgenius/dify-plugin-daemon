...
func (c *ControlPanel) LaunchLocalPlugin(
	...
	// To avoid cross-pod races on Python venv creation, guard InitEnvironment with a Redis-based distributed lock.
	{
		...
		log.Info("acquiring distributed init lock for %s (expire=%s)...", pluginUniqueIdentifier.String(), expire.String())
		...
		defer func() {
			if unlockErr := cache.Unlock(lockKey); unlockErr != nil {
				log.Warn("failed to release distributed init lock for %s: %s", pluginUniqueIdentifier.String(), unlockErr.Error())
			} else {
				log.Info("released distributed init lock for %s", pluginUniqueIdentifier.String())
			}
		}()
