...
const sessionCacheTTL = time.Minute * 30
...
// SessionCacheKey returns the Redis key used to store a session.
func SessionCacheKey(id string) string {
	return sessionKey(id)
}

// SessionCacheErrorKind classifies cache lookup failures for session diagnostics.
func SessionCacheErrorKind(err error) string {
	switch {
	case errors.Is(err, cache.ErrNotFound):
		return "not_found"
	case errors.Is(err, cache.ErrDBNotInit):
		return "redis_not_initialized"
	default:
		return "read_failed"
	}
}

// SessionCacheErrorHint returns an operator-facing hint for a cache error kind.
func SessionCacheErrorHint(kind string) string {
	switch kind {
	case "not_found":
		return "session cache key is missing; possible causes include session closed and deleted, TTL expired, wrong session_id, or Redis key prefix/DB mismatch"
	case "redis_not_initialized":
		return "Redis client is not initialized; check daemon Redis configuration and startup logs"
	default:
		return "Redis read or session decode failed; check Redis connectivity, auth, timeout, and cached value compatibility"
	}
}
...
func NewSession(payload NewSessionPayload) *Session {
	...
	if !payload.IgnoreCache {
		cacheKey := sessionKey(s.ID)
		if err := cache.Store(cacheKey, s, sessionCacheTTL); err != nil {
			log.WarnContext(
				s.RequestContext(),
			...
			log.WarnContext(
				...
				"session_id", s.ID,
				"cache_key", cacheKey,
				"cache_ttl_seconds", int(sessionCacheTTL.Seconds()),
				"cluster_id", s.ClusterID,
				"plugin_unique_identifier", s.PluginUniqueIdentifier.String(),
			)
		}
...
func GetSession(id string) (*Session, error) {
	...
	if session == nil {
		...
		cacheKey := sessionKey(id)
		session, err := cache.Get[Session](cacheKey)
		if err != nil {
			kind := SessionCacheErrorKind(err)
			return nil, fmt.Errorf(
				"failed to get session info from cache: cache_key=%s cache_error_kind=%s hint=%s: %w",
				cacheKey,
				kind,
				SessionCacheErrorHint(kind),
				err,
			)
		}
...
func DeleteSession(payload DeleteSessionPayload) {
	...
	if !payload.IgnoreCache {
		...
		if _, err := cache.Del(cacheKey); err != nil {
			...
			log.InfoContext(
				session.RequestContext(),
				"session info deleted from cache",
				"session_id", payload.ID,
				"cache_key", cacheKey,
				"cluster_id", session.ClusterID,
				"plugin_unique_identifier", session.PluginUniqueIdentifier.String(),
...
const (
	...
	PLUGIN_IN_STREAM_EVENT_CANCEL   PLUGIN_IN_STREAM_EVENT = "cancel"
)
